...
- **Status as of 2026-09-17:** **Phase 1 — Core Platform MVP** and **Phase 2 — AI Agent System are both fully complete** (all of `DESIGNSYS`, `AUTH`, `PROF`, `LAND`, `CHAT`, `MEM`, `DASH`, and all nine `AGENTS-01..09` — see `TASK_BOARD.md`/`PROJECT_STATE.md`). `/chat` now dispatches through the real multi-agent Orchestrator for both guest and authenticated callers. **Phase 3 — External Data Integration, Wave 1, is now elaborated to Task level** (`Module: INTEG`, `ATLAS-P3-INTEG-01..06` — see `WORK_BREAKDOWN_STRUCTURE.md` §Phase 3), documentation-only, per the project owner's approved Q1–Q5 (`DESIGN_BIBLE_AMENDMENTS.md` Amendment 011). **No Phase 3 task is authorized for implementation yet** — this elaboration only makes the tasks definable and Ready; each still needs its own explicit "Execute" instruction, same as every other task in this project's history. Wave 2 (`Module: DOMAIN-AGENTS`) remains unelaborated by design (Q5) — a separate future planning pass, only after Wave 1 is built and verified.
- **Implementation of the next feature task is still not separately authorized by this file.** This prompt tells a session *how* to start; the "Execute `<TASK-ID>`" instruction you give alongside it is the authorization for that specific task. Recommended next task: `ATLAS-P3-INTEG-01` (adapter foundation — timeout, retry, provider-scoped rate limiting, caching, validation/error normalization, monitoring hooks; extends `AGENTS-03`'s Tool Service, does not rebuild it; see `TASK_BOARD.md`).
- `ATLAS-P3-INTEG-01` through `06` are all backend/AI-layer only — no UI component is created or consumed by any of them, so step 6 (`COMPONENT_OWNERSHIP_MATRIX.md`) can be skipped for the whole module. Whether step 7 (`INFRASTRUCTURE_BASELINE.md`) applies is a judgment call per task — `INTEG-01` touches `app/core/rate_limit.py`/`app/core/redis.py` only if its own implementing session finds a genuine reuse need (reported, not assumed), so that session should check §8 first; `INTEG-02` through `06` are new-file-only and likely don't need it.
- **Provider selection for Maps/Weather is deliberately not pre-decided** (`DESIGN_BIBLE_AMENDMENTS.md` Amendment 011, Q2/Q3) — each adapter's contract must be provider-agnostic (mirroring `ai.providers.base.LLMProvider`/`ai.rag.embeddings.EmbeddingProvider`'s own existing pattern); the concrete backend, if any, is that task's own implementing session's choice. **No live network call against any external provider is expected without a real credential** (Q4) — every `INTEG-02`–`06` task's Definition of Done is satisfiable with fake/mock provider fixtures alone.
- If you're running many sessions in parallel, check `MASTER_RULES.md` §28 before assuming two tasks can run side by side. `WORK_BREAKDOWN_STRUCTURE.md` §Phase 3's own Parallelization note: `INTEG-02` through `06` may all run in parallel once `INTEG-01` is Done (disjoint new files, no cross-dependency) — `INTEG-01` itself blocks all five and must go first.
...
**LOCK STATUS:**
**IMMUTABLE (baseline) — approved 2026-07-22. Changed only via the Amendment History process (§23), never by freeform edit. Step 6 and the related "Do not recreate" line approved 2026-08-13. Step 7, dependency resolution, task-group handling, and incremental-output instructions approved 2026-08-16 (`DESIGN_BIBLE_AMENDMENTS.md`, Amendment 009). "Notes for the Human Operator" refreshed 2026-09-09 for Phase 1 completion and the Phase 2 AGENTS elaboration (`DESIGN_BIBLE_AMENDMENTS.md`, Amendment 010), and again 2026-09-17 for Phase 2 completion and the Phase 3 INTEG Wave 1 elaboration (`DESIGN_BIBLE_AMENDMENTS.md`, Amendment 011) — status refreshes of living operator guidance, not changes to the numbered instruction steps themselves.**
