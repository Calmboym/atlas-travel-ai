...
| AIQuickAccess | Unclaimed — `LAND-01`, `CHAT-01`, and now `DASH-01` have all shipped without taking it | — | LAND, DASH, CHAT, Phase 2+ Destination/Hotel/Flight | Button/IconButton | Not built | **Explicitly declined by `LAND-01` (2026-08-29)**, again by **`CHAT-01` (2026-09-01)**, and again by **`DASH-01` (2026-09-08)**. LAND-01's reasoning: a persistent cross-page AI quick-access trigger needs an actual chat surface to open, which didn't exist yet. `CHAT-01`'s reasoning: adding a `Navbar` slot was `DESIGNSYS-03` territory, outside CHAT-01/02's own file boundaries. `DASH-01` *did* touch `Navbar`'s slots (`userSlot`/`notificationsSlot`, for `ProfileMenu`/`NotificationCenter`) but found no documented requirement calling for a *third*, AI-specific global slot beyond those two plus Dashboard's own in-page "New trip"/"Continue chat" `QuickActions` — every AI entry point `INFORMATION_ARCHITECTURE.md` §AI Entry Points actually lists (Dashboard, Chat) is already reachable without it. Remains genuinely unclaimed, not silently dropped — the next task with a concrete cross-page need for it (e.g. a future Destination/Hotel/Flight page wanting a floating AI trigger) is the natural owner. |
| NotificationCenter | `DASH-01` | `DASH-01` | Global header, DASH | Toast, Badge | ✅ Built (2026-09-08) — `frontend/components/layout/notification-center.tsx` | Honest empty-state shell only (`23_NOTIFICATION_COMMUNICATION_EXPERIENCE.md` §Empty Notification Center: "You're all caught up") — no real notification data source exists until a Phase 3+ NOTIF task feeds real items into the same panel |
| ProfileMenu | `DASH-01` | `DASH-01` | Global header | Avatar, Popover | ✅ Built (2026-09-08) — `frontend/components/layout/profile-menu.tsx` | Only links to routes that exist today (Dashboard, Profile, Settings) plus a real Sign out (`logoutRequest()` newly added to `lib/api/auth.ts` — the backend endpoint existed since `AUTH-07` but had no frontend caller anywhere). My Trips/Saved/Help still omitted, same reasoning `PROF-03` originally gave — no page exists for any of them yet |
...
| QuickActions | `DASH-01` | `DASH-01` | DASH, any page with a FAB | FAB, Sheet | ✅ Built (2026-09-08) — `frontend/components/shared/quick-actions.tsx` | Rendered as an inline action row on Dashboard, not inside a `FloatingActionButton` — deliberately data-driven and copy-free (same pattern as `EmptyState`/`ErrorState`: every label/href/icon supplied by the caller), so a future consumer can wrap it in a FAB container without forking it |
| ConnectionStatus, RetryCard | `DASH-01` | `DASH-01` | Chat, Trip Details, Dashboard | Alert | ✅ Built (2026-09-08) — `frontend/components/shared/connection-status.tsx`, `retry-card.tsx` | First real (non-hypothetical) usage: `DashboardPageContent`'s own `GET /auth/me` + `GET /profile/me` fetch — `ConnectionStatus` shows a "reconnecting" state and `RetryCard` offers Retry when it fails, satisfying the "once there's an actual connection worth monitoring" bar the prior two declines were waiting on |
...
| Dashboard-specific | ✅ Done (2026-09-08). `DashboardPageContent` (`components/dashboard/dashboard-page-content.tsx` — data fetching, loading/error states, section composition), `DashboardHero` (`dashboard-hero.tsx` — the Phase 1 stand-in for `18_DASHBOARD_EXPERIENCE.md`'s Travel Summary Hero, which the doc itself scopes to "when an active trip exists"; implements §Default Landing's welcome/continue-conversation states instead), `DashboardSuggestedPrompts` (`dashboard-suggested-prompts.tsx`), `DashboardCapabilities` (`dashboard-capabilities.tsx`), `DashboardEmptySection` (`dashboard-empty-section.tsx` — reused for both "Your trips" and "Recommendations", both honestly empty in Phase 1) | `DASH-01` | Card, GlassSurface (`atlas-glass-1` utility class), Skeleton, Button, Heading/Paragraph, ScrollReveal | QuickActions, ConnectionStatus, RetryCard (all three built by this same task, see §4) | Trip Service / Recommendation Agent data doesn't exist until Phase 2+ (`DEPENDENCY_GRAPH.md` §4/§5) — Travel Summary Hero, Travel Timeline, and every trip-data-backed widget (Budget/Weather/Checklist) are explicitly not built; fabricating that data would violate `BRAND_GUIDELINES.md` §13 |
