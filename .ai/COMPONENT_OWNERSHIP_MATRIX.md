...
| AIQuickAccess | `CHAT-01` (LAND-01 shipped first but explicitly declined — see rationale) | — | LAND, DASH, CHAT, Phase 2+ Destination/Hotel/Flight | Button/IconButton | Not built | **Explicitly declined by `LAND-01` (2026-08-29)**, not claimed despite shipping before `CHAT-01`: a persistent cross-page AI quick-access trigger needs an actual chat surface to open, which doesn't exist yet — building it now would mean inventing UI for a destination that isn't real. `LAND-02`'s own `AISearchBox` already covers Landing's primary AI entry point in the meantime. Next candidate owner: `CHAT-01`, once `/chat` is real. |
